<!--
 * @Author: Qing Shi
 * @LastEditTime: 2022-04-30 22:58:17
 * @Knowledge: 
 * @Description: 
 * @Attention: 
-->
  ...
  <div>
    ...
    <div id="mainHistogram">
      <div
        ...
        :key="ty"
        :id="'His' + ty"
        v-bind:style="{ border: 'solid 1px rgba(0, 0, 0, 0.5)', width: wd, height:hs, position: 'absolute', left: '10px', transform: 'translate(' + (index == 10 ? 'calc(50% + 10px)' : index % 2 ? 'calc(100% + 10px)' : '0') + ',' + 'calc(' + ((Math.floor(index / 2)) * 100) + '% + ' + ((Math.floor(index / 2)) * 5) + 'px' + ' + 10px)' + ')' }"
      >
    ...
    RectBrush (hisData, hisType) {
      ...
      function brushed ({ selection }) {
        ...
        if (filePath[13] == 'D') {
          ...
          for (let i = 0; i < 9; ++i) {
            if (x0 < _this.xAxis((i + 1) / 10) + 40 + 6 && _this.xAxis((i + 1) / 10) + 40 + 6 < x1) {
