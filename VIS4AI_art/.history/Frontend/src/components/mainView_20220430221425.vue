<!--
 * @Author: Qing Shi
 * @LastEditTime: 2022-04-30 22:14:25
 * @Knowledge: 
 * @Description: 
 * @Attention: 
-->
  ...
  methods: {
    ...
    drawScatter (scatterData) {
      ...
      let scatter_g = d3.select("#mainViewPicSvg").append('g')
        ...
        .attr('transform', `translate(${this.mainWidth / 2}, ${this.mainHeight / 2})`);
      let tooltip = d3.select("#mainViewPicSvg").append("div")
      ...
      let circles = scatter_g
        ...
        .on('mouseover', d => {
          if (d3.select("#nodePic").attr("sid") != 1) {
            ...
            let tx = selData.x0 * this.chartRadius;
            let ty = selData.y0 * this.chartRadius;
            tooltip.html("<img style='height: 50px; width: 100px;' src='" + "/src/assets" + selData.path.slice(1) + "'>")
            .style("left", tx + "px")
            .style('top', ty + 'px')
          }
        })
