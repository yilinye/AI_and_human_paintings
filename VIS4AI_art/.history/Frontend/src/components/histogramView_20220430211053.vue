  ...
  <div>
    ...
    <div id="mainHistogram">
      <div
        ...
        :key="ty"
        :id="'His' + ty"
        v-bind:style="{ border: 'solid 1px rgba(0, 0, 0, 0.5)', width: wd, height:hs, position: 'absolute', left: '2px', transform: 'translate(' + (index == 10 ? 'calc(50% + 5px)' : index % 2 ? 'calc(100% + 5px)' : '0') + ',' + 'calc(' + ((Math.floor(index / 2)) * 100) + '% + ' + ((Math.floor(index / 2)) * 5) + 'px' + ' + 8px)' + ')' }"
      >
