<!--
 * @Author: Qing Shi
 * @LastEditTime: 2022-04-30 23:22:11
 * @Knowledge: 
 * @Description: 
 * @Attention: 
-->
      ...
      let circles = scatter_g
        ...
        .on('mouseover', d => {
          if (d3.select("#nodePic").attr("sid") != 1) {
            ...
            d3.select('#cir' + selData.pid).attr('r', 8).attr('fill', 'yellow').attr('border-width', 10);
            // this.setSelect(selData, this.typeName);
            let tx = selData.x0 * this.chartRadius + this.mainWidth / 2 - 70;
            let ty = selData.y0 * this.chartRadius + this.mainHeight / 2 - 80;

            let tooltip = d3.select("#mainViewPic").append("div")
              .attr("id", "tooltip") //用于css设置类样式
              .attr("display", 'none');//新建一个tooltip
            tooltip.html("<img style='height: 70px; width: 140px; border:solid 1px black;' src='" + "/src/assets" + selData.path.slice(1) + "'>")
              .style("left", tx + "px")
              .style('top', ty + 'px')
        ...
        .on('mouseout', d => {
          let selData = d.srcElement.__data__;
          d3.select('#cir' + selData.pid).attr('r', 2).attr("fill", this.colormap[selData.class.toLowerCase()]);
          d3.select("#tooltip").remove();
          if (d3.select("#nodePic").attr("sid") != 1) {
